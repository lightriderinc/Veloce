...
\begin{description}[style=nextline]
  ...
  (\code{wc\_SetSeed\_Cb()}, per wolfSSL FIPS FAQ \S2) drawing CPU RDSEED hardware
  entropy (x86-64) and applying SP~800-90B RCT/APT health tests to every seed block
  in-process, fail-closed. No ESV
...
Entropy source & Lightrider local entropy provider: custom generate-seed callback wired
  via \code{wc\_SetSeed\_Cb()}, CPU RDSEED hardware entropy + SP~800-90B RCT/APT health
  tests, fail-closed; OS DRBG output only by explicit configuration, reported as an
  unvalidated SP~800-90C chain & The FIPS module itself makes no entropy claim and requires an external
...
Entropy & Lightrider local entropy provider (CPU RDSEED hardware entropy via custom
  generate-seed callback), SP~800-90B RCT/APT health tests on every seed block, fail-closed
...
SP 800-90B & Entropy source health tests & RCT (cutoff 4) + APT (13/512) at
  $H = 8$ bits/sample on the RDSEED seed stream, 1024-sample startup test (engineering
  assurance; no ESV certificate exists for v5.2.1, legacy IG~9.3.A applies) \\
SP 800-90C & RBG construction & RDSEED entropy source seeding the module DRBG directly;
  the optional OS DRBG source is an RBGC chain and carries no security-strength claim \\
...
\begin{tikzpicture}[
  ...
  \node[box, minimum width=13em] (ent) at (-0.1,-8.5)
    {\icWave\ Lightrider entropy provider\\\scriptsize CPU RDSEED entropy + SP 800-90B RCT/APT, fail-closed};

  % --- FIPS boundary ---
  \node[draw=fipsgold, dashed, thick, rounded corners=3pt, fit=(wc), inner sep=5pt]
        (fipsboundary) {};
  \node[fill=white, text=fipsgold, font=\scriptsize\sffamily,
        inner xsep=2pt, inner ysep=0pt, anchor=south west]
        at ([xshift=5pt]fipsboundary.north west) {FIPS boundary};
...
Lightrider entropy provider & C++ (agent) & Custom generate-seed callback: CPU RDSEED
  entropy acquisition (OS DRBG output only by explicit configuration), SP~800-90B RCT/APT
  health tests, fail-closed delivery via \code{wc\_SetSeed\_Cb()}; live entropy-spec
  reporting including source kind, RBG construction, and strength claim. \\
...
\begin{tikzpicture}[
  ...
  arrow/.style={-{Stealth[length=2mm]}, thick},
  node distance=0.5cm]
  \node[box] (src) {\icWave\ \textbf{CPU RDSEED entropy source}\\\scriptsize x86-64 \code{RDSEED}: conditioned SP 800-90B noise-source output; OS DRBG only by explicit configuration};
  \node[box, below=of src] (cond) {\icShield\ \textbf{Lightrider entropy health tests}\\\scriptsize SP 800-90B RCT (cutoff 4) + APT (13/512), $H=8$ bits/sample; 1024-sample startup test + continuous};
...
\code{wc\_InitRng()}. The callback acquires seed material from the processor's RDSEED
instruction (conditioned output of the CPU's SP~800-90B noise source, delivered directly
to the module DRBG so that no second DRBG sits in the seed path), runs the SP~800-90B
RCT/APT health tests over every block, and returns it only on pass; any acquisition or health-test failure returns an error, and because the
...
\textbf{Seed source selection (revised 2026-09-17).} The configuration key
\code{entropy.source} selects \code{rdseed} (default) or \code{os-drbg}; there is no
runtime fallback between them. \code{os-drbg} reads \code{getrandom()} on Linux and
\code{BCryptGenRandom} on Windows, which return the operating system DRBG's output.
Seeding the module DRBG from that output forms an SP~800-90C RBGC construction, and
where the operating system RBG is not validated for the operational environment no
security strength can be claimed for the chain; the same is true of the module's
portable \code{wc\_GenerateSeed} callback, which reads \code{/dev/urandom}. The
\code{os-drbg} source therefore exists only for processors without RDSEED (arm64) and
for development, and \code{validation\_status()} reports it as
\code{rbg\_construction: SP 800-90C RBGC} with \code{security\_strength\_claim: none}.
The RDSEED source is reported as an SP~800-90B entropy source seeding the DRBG directly;
its health-test parameters assume the processor vendor's full-entropy rating
($H = 8$ bits per 8-bit sample, $\alpha = 2^{-20}$), and a formal entropy assessment or
ESV certificate for the RDSEED source on each operational environment remains an open
vendor item. When RDSEED is absent and \code{os-drbg} is not configured, the agent fails
closed.
...
external seed source, reported in \code{validation\_status()} as
\code{esv\_certified:\ false}, \code{source\_kind}, \code{rbg\_construction}, and
\code{security\_strength\_claim:\ none}. The Lightrider RCT/APT layer is engineering
assurance and is never represented as an ESV credit. Per wolfSSL: ``testing is the only confidence when it comes to FIPS''; the
...
entropy provider (CPU RDSEED entropy, health-tested per block; OS DRBG output only by
explicit configuration). No external hardware entropy device is integrated in V1. Two quantum-entropy integrations follow post-V1: the
...
Policies accepted by the service are \code{fastest\_available} (Free tier),
\code{highest\_quality}, and \code{quantum\_verified}; the receipt signature is computed
over the canonical receipt bytes (sorted keys, \code{signature} removed, compact JSON).
...
\textbf{Design.}
\begin{enumerate}
  \item The agent (when \code{ems.mode:\ enabled} and \code{entropy\_mixin:\ on})
        periodically fetches an entropy packet from EMS over HTTPS
        (\code{POST /v1/entropy/request}) using the TLS client of the loaded FIPS module,
        with the ISRG Root X1 anchor pinned (\code{ems.ca\_file} overrides), an optional
        Bearer API key (the Free tier is keyless on \code{fastest\_available}), and a
        receipt verified against the pinned service key (\code{ems.pubkey\_hex};
        Ed25519 via the PQC provider's verify-only implementation, or ML-DSA-65) after
        canonicalization identical to the service (sorted keys, signature removed,
        compact JSON); freshness (\code{ems.max\_age\_s}), packet length, health flags,
        and \code{raw\_entropy\_stored} are checked. mTLS device enrollment remains the
        target transport hardening once EMS supports it.
  \item The packet is mixed into the wolfCrypt DRBG as the SP~800-90A
        \emph{instantiation nonce}: certificate \#4718 exports no additional-input or
        reseed entry point, so the agent re-instantiates the DRBG through
        \code{wc\_InitRngNonce()}, which draws fresh seed-source entropy through the
        registered callback and mixes the packet as nonce via Hash\_df. The packet is
        cryptographically mixed in but contributes \emph{zero} to the credited entropy
        claim. The local Lightrider entropy provider remains the sole seed. The FIPS
        certificate claims are unaffected.
...
\begin{enumerate}
  \item \textbf{Show the live entropy streaming specs.} One table, populated from
        ...
        counters (RCT cutoff 4, APT 13/512, blocks verified, failures: always zero or
        the agent is degraded); source kind, RBG construction, and strength claim; last seed/reseed time; and, when the mix-in is on,
...
FIPS \& entropy & wolfCrypt test suite; startup + conditional self-tests; Lightrider
  seed-callback health tests (RCT/APT pass and induced-failure); seed-source reporting
  (RDSEED direct or explicitly labelled OS DRBG chain, no over-claim); induced
