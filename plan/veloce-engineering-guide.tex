...
Version & 0.2 (implementation-aligned review) \\
Date & August 10, 2026 \\
Prepared for & Martin Guo, Quantum Applied Scientist \\
Status & Linux reference implementation complete; Windows/macOS work planned \\
\bottomrule
\end{tabular}
\vfill
...
The checked-in implementation is currently Linux x86-64; native Windows and macOS
deliverables remain separate platform tracks in this same repository.
\end{minipage}
\vspace{2em}
\end{titlepage}
...
\textbf{Implementation snapshot (2026-08-10).} The checked-in Linux implementation
contains the C++ agent, pure-Python client, Rust CLI, native Rust qSearch source/system
scanner, CBOM outputs, and 33-test gate battery. The current source scanner is a
single-threaded recursive literal-pattern engine, not yet the parallel collector design
described as the target below. Hybrid-TLS policy selection exists, but the TLS data plane,
native Windows/macOS transports, native packages, dependency/PCAP collectors, and EMS
network client are not implemented. This distinction is normative: target requirements
must not be reported as delivered functionality.
...
\begin{description}[style=nextline]
  ...
  fail-closed). The agent provides ML-KEM-768 key establishment and ML-DSA signatures,
  consumed from a pure-Python SDK over an authenticated local IPC channel. Hybrid TLS 1.3
  policy selection is present; the sample handshake/data plane remains a G3 deliverable.
\end{description}
...
Platforms & Linux x86-64 implemented; Windows 11 x86-64 next; macOS separate track &
...
\textbf{Wiring.} The module itself makes no entropy claim. After loading the module and
checking \code{wolfCrypt\_GetStatus\_fips()}, the implemented agent runs the wolfEntropy
on-demand health test, registers \code{wc\_GenerateSeed} with
\code{wc\_SetSeed\_Cb()}, and instantiates the FIPS DRBG with \code{wc\_InitRng()}.
The build uses \code{-{}-enable-wolfEntropy=nofallback}, so initialization or generation
fails closed rather than silently falling back to \code{/dev/urandom}. The agent exposes
...
\begin{itemize}
  ...
  \item The licensed source package does include Windows FIPS build material:
        \code{IDE/WIN10/wolfssl-fips.sln}, \code{wolfssl-fips.vcxproj}, and
        \code{user\_settings.h}. Its README says the supplied library is currently built
        statically and documents special in-core integrity linker settings.
  \item As checked on 2026-08-10, wolfSSL lists \textbf{Windows 10 Enterprise on Intel
        Core i5-1345U} (with and without PAA) and \textbf{Windows 11 Pro on Intel Core
        i7-1260P @ 2.10 GHz} as \emph{pending operational environments}. NIST's CAVP
        record A4308 already enumerates these algorithm-test OEs, but the current CMVP
        certificate \#4718 security policy does not list them in its approved tested-OE
        table. CAVP algorithm testing and a pending OE must not be represented as current
        CMVP module/OE approval. The earlier i7-1255U statement referred to a separate
        future module and was incorrect for this claim.
  \item The CMVP entry carries the caveat ``no assurance of the minimum strength of
        generated SSPs''; entropy assurance therefore depends on the separately assessed
        entropy source. \dep{Obtain wolfSSL confirmation when the two Windows OEs are
        added to the published \#4718 security policy, plus the approved dynamic-library
        configuration and exact v5.2.1 \code{user\_settings.h}. Until then Windows is a
        development target, not a Veloce validated deployment claim.}
\end{itemize}
...
\midrule
Windows FIPS dynamic-library configuration + ESV OE applicability unconfirmed; the
  Windows 10/i5-1345U and Windows 11/i7-1260P environments are pending and not yet in
  the current \#4718 security-policy tested-OE table & M & H & Engage wolfSSL at S0;
  treat CAVP records as algorithm evidence only; make no Windows validated-deployment
  claim until the CMVP OE update and exact build guidance are published. \\
...
Platform \code{x64}, per-wolfSSL-approved configuration only. Local inspection confirms
the solution, project, and \code{user\_settings.h} exist. The included README currently
says to build a static library and documents integrity-sensitive linker settings; Veloce's
dynamic-loading requirement therefore remains unresolved. wolfSSL lists Windows 10
Enterprise/i5-1345U (with and without PAA) and Windows 11 Pro/i7-1260P as pending OEs;
the A4308 CAVP record is not by itself CMVP module/OE approval.
\dep{Obtain the approved DLL configuration, exact user guide/settings, and published
\#4718 OE update before enabling the Windows FIPS claim.} Then implement the named-pipe
transport, restricted DLL loading, Windows service, Windows collectors, MSI, and the same
hash-verify-record flow behind platform adapters.
