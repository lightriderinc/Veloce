// Standalone SHA-256 used to verify library files against the recorded
// build hash BEFORE they are loaded with dlopen (spec 8, library loading).
// The FIPS module cannot verify its own file before it is loaded, so this
// small independent implementation performs the pre-load check; the module
// then runs its own in-core HMAC-SHA256 integrity self-test at load.
#pragma once
...
public:
    ...
    void final(uint8_t out[32]) {
        uint64_t bits = len_ * 8; // captured before padding is appended
        uint8_t pad = 0x80;
        update(&pad, 1);
        uint8_t zero = 0;
...
// SHA-256 of a file, hex encoded; empty string on read failure.
inline std::string fileSha256Hex(const std::string& path) {
