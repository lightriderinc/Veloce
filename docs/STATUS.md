...
| G1 | Entropy pipeline + ML-KEM/ML-DSA pass KATs and failure-injection on both OSes | GREEN on Linux: wolfEntropy wired via wc_SetSeed_Cb, nofallback fail-closed verified (DRBG refuses without seed source); PQC PCT + negative tests at startup and in gate battery. Windows: pending (vendor DLL config open item) |
...
Gate battery: `bash scripts/run_gates.sh` (25 tests, all passing).
...
| Reseller evaluation package, EULA | LICENSE draft present (legal review required) |
...
1. PQC provider source. The commercial FIPS bundle strips SHAKE from
   ...
   only under the commercial agreement. [VENDOR: confirm coverage.]
...
public-tree PQC sources (item 1 above).
